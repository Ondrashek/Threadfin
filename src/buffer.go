			...
			if stream, ok := playlist.Streams[streamID]; ok {
				...
				for { // Loop 2: Temporäre Datein sind vorhanden, Daten können zum Client gesendet werden
					...
					if ok {
						...
						case <-ctx.Done():
                            showInfo("Streaming Status:kill client done")
						...
						default:
							if c, ok := BufferClients.Load(playlistID + stream.MD5); ok {
								...
								if clients.Error != nil {
                                    ...
                                    showInfo("Streaming Status:kill client default")
									killClientConnection(streamID, playlistID, false)
									return
								}
					...
					if _, err := bufferVFS.Stat(stream.Folder); fsIsNotExistErr(err) {
                        showInfo("Streaming Status:kill client fsisnot")
						killClientConnection(streamID, playlistID, false)
						return
					}
					...
					for _, f := range tmpFiles {
						...
						if _, err := bufferVFS.Stat(stream.Folder); fsIsNotExistErr(err) {
                            showInfo("Streaming Status:kill client fsisnot2")
							killClientConnection(streamID, playlistID, false)
							return
						}
							...
							if err == nil {
								...
								if err == nil {
									...
									*/
									...
									if err != nil {
										file.Close()
                                        showInfo("Streaming Status:kill client fileclose")
										killClientConnection(streamID, playlistID, false)
										return
									}
                ...
                showInfo("Streaming Status:kill client nicht")
				killClientConnection(streamID, stream.PlaylistID, false)
				showInfo(fmt.Sprintf("Streaming Status:Playlist: %s - Tuner: %d / %d", playlist.PlaylistName, len(playlist.Streams), playlist.Tuner))
				return
...
func thirdPartyBuffer(streamID int, playlistID string, useBackup bool, backupNumber int) {
    ...
    showInfo("Streaming Status: thirdparty")
	if p, ok := BufferInformation.Load(playlistID); ok {
		...
		default:
            showInfo("Streaming Status: default")
			return
		}
        ...
        showInfo("Streaming status:cmd.Wait")

		go func() {

            showInfo("Streaming status:go func")
		...
		if err != nil {
            showInfo("Streaming status:panic")
			panic(err)
		}
		defer f.Close()
		...
		go func() {
			...
			for {
				...
				select {
				case <-t:
                    showInfo("Streaming status:return")
					return
				default:
                    thirdPartyBuffer(streamID, playlistID, true, 1)
					//t <- timeout
				}
		...
		for {
			...
			case timeout := <-t:
				if timeout >= 20 && tmpSegment == 1 {
					...
					ShowError(err, 4006)
                    showInfo("Streaming Status: ERROR")
					addErrorToStream(err)
					cmd.Wait()
					f.Close()
					return
